 ...
 * 一、BigDecimal概述
 *
 * ​ Java在java.math包中提供的API类BigDecimal，用来对超过16位有效位的数进行精确的运算。双精度浮点型变量double可以处理16位有效数，但在实际应用中，
 * 可能需要对更大或者更小的数进行运算和处理。一般情况下，对于那些不需要准确计算精度的数字，我们可以直接使用Float和Double处理，但是Double.valueOf(String)
...
public class TestBigDecimal {
	...
	public void test01(){
		...
		System.out.println("-------- bigD :"+bigDecimal);

		System.out.println("=============================");
		BigDecimal bigDecimal1 = new BigDecimal("0.1");
		System.out.println("---------bigD2 :"+bigDecimal1);
		 ...
		 *
		 * 以利用BigDecimal对货币和百分比格式化为例。首先，创建BigDecimal对象
		 * ，进行BigDecimal的算术运算后，分别建立对货币和百分比格式化的引用，
		...
		percent.setMaximumFractionDigits(3);//表示百分比小数点最多三位

		BigDecimal loanAmount = new BigDecimal("15000.48");//贷款金额
		BigDecimal interestRate = new BigDecimal("0.008");//利率
		BigDecimal insert = loanAmount.multiply(interestRate);//相乘

		System.out.println("贷款金额:\t" + currency.format(loanAmount));
		System.out.println("利率:\t" + percent.format(interestRate));
		System.out.println("利息:\t" + currency.format(insert));

		System.out.println("-------------------BigDecimal格式化保留2为小数，不足则补0：--------------------");
		 ...
		 *     复杂的运算时尤为明显。故一般精度的计算没必要使用BigDecimal。
